...
def fuse_scores(
    ...
    ml_threshold: float = ML_FUSION_THRESHOLD,
) -> pd.DataFrame:
    """Combine Z-score, IF, AE scores with physics rules into a final flag.

    Three pathways combined with OR:
      Pathway 1 - strict_rules   : HDF, PWF, OSF always flag (~100% precise)
      Pathway 2 - twf_confirmed  : TWF rule + IF score > 0.50 (cuts false alarms)
      Pathway 3 - ml_fusion      : combined ML score > 0.70 (safety net)
    """
    ...
    ml_fusion     = combined > ml_threshold
